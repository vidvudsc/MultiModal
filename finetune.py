...
def clean_short_text(text: str, max_words: int = 90) -> Optional[str]:
    text = " ".join(str(text).strip().split())
    if not text:
        return None
    if any(x in text.lower() for x in ["as an ai", "language model", "training data", "<|user|>", "<|assistant|>"]):
        return None
    chars = [c for c in text if not c.isspace()]
    if chars and sum(ord(c) < 128 for c in chars) / len(chars) < 0.96:
        return None
    words = text.split()
...
def encode_chat_sample(
    ...
    user_ids = token_ids(tokenizer, f"<|user|> {user.strip()}\n<|assistant|> ")
    answer_ids = token_ids(tokenizer, assistant.strip()) + [eos_id]

    if image is None:
        prefix = [bos_id] + user_ids
        available = cfg.max_seq_len - len(prefix)
    ...
    n_patches = (cfg.image_size // cfg.patch_size) ** 2
    prefix = [bos_id, image_id]
    mid = [image_end_id] + user_ids
    available = cfg.max_seq_len - len(prefix) - n_patches - len(mid)
    answer_ids = answer_ids[: max(0, available)]
    ids = prefix + [image_id] * n_patches + mid + answer_ids
    labels = [-100] * (len(prefix) + n_patches + len(mid)) + answer_ids
    return {
        "input_ids": torch.tensor(ids, dtype=torch.long),
        "labels": torch.tensor(labels, dtype=torch.long),
        "image": image_transform(image, cfg.image_size, train=False),
        "image_position": len(prefix),
    }
...
TEXT_PROMPTS = [
    ...
    "Continue with a short, clear story.",
...
def row_image_and_captions(row: Dict[str, Any]) -> Tuple[Optional[Image.Image], List[str]]:
    ...
    return image, [" ".join(str(c).split()) for c in captions if str(c).strip()]
...
class SFTDataset(IterableDataset):
    ...
    def jsonl_rows(self) -> Iterator[Dict[str, Any]]:
        paths = [Path(x) for x in self.args.jsonl]
        base_dirs = [Path(x) for x in self.args.image_base_dir]
        rows = []
        for path in paths:
        ...
        if not rows:
            raise RuntimeError("No rows found in JSONL input.")
        rng = random.Random(self.args.seed + 100)
        while True:
            rng.shuffle(rows)
            for path, line in rows:
                ...
                assistant = row.get("assistant") or row.get("answer") or row.get("response")
                user = clean_short_text(user, max_words=50)
                assistant = clean_short_text(assistant, max_words=self.args.max_answer_words)
                if not user or not assistant:
                    continue
                image = None
                image_ref = row.get("image")
                if image_ref:
                    candidates = [Path(image_ref), path.parent / image_ref]
                    for base in base_dirs:
                        candidates.append(base / image_ref)
                        candidates.append(base / Path(image_ref).name)
                    ...
                    if image is None:
                        continue
                yield encode_chat_sample(self.tokenizer, self.cfg, user, assistant, image=image)

    def tiny_stories(self) -> Iterator[Dict[str, Any]]:
        ds = self.load_dataset("roneneldan/TinyStories", split="train", streaming=True)
        ds = self._shard_shuffle(ds, self.args.shuffle_buffer, self.args.seed + 1)
        rng = random.Random(self.args.seed + 101)
        for row in ds:
            answer = clean_short_text(row.get("text", ""), max_words=110)
            if answer:
                yield encode_chat_sample(self.tokenizer, self.cfg, rng.choice(TEXT_PROMPTS), answer)

    def coco(self) -> Iterator[Dict[str, Any]]:
        ds = self.load_dataset("jxie/coco_captions", split="train", streaming=True)
        ds = self._shard_shuffle(ds, self.args.image_shuffle_buffer, self.args.seed + 2)
        rng = random.Random(self.args.seed + 102)
        for row in ds:
            image, captions = row_image_and_captions(row)
            if image is None:
                continue
            captions = [clean_short_text(c, max_words=45) for c in captions]
            captions = [c for c in captions if c]
            if captions:
                yield encode_chat_sample(self.tokenizer, self.cfg, rng.choice(IMAGE_PROMPTS), rng.choice(captions), image=image)

    def flickr(self) -> Iterator[Dict[str, Any]]:
        ds = self.load_dataset("Mozilla/flickr30k-transformed-captions", split="test", streaming=True)
        ds = self._shard_shuffle(ds, self.args.image_shuffle_buffer, self.args.seed + 3)
        rng = random.Random(self.args.seed + 103)
        for row in ds:
            image, captions = row_image_and_captions(row)
            if image is None:
                continue
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        streams: List[Tuple[float, Iterator[Dict[str, Any]]]] = []
        if self.args.jsonl and self.args.mix_jsonl > 0:
            streams.append((self.args.mix_jsonl, self.jsonl_rows()))
        if self.args.mix_tinystories > 0:
            streams.append((self.args.mix_tinystories, self.tiny_stories()))
        if self.args.mix_coco > 0:
            streams.append((self.args.mix_coco, self.coco()))
        if self.args.mix_flickr > 0:
            streams.append((self.args.mix_flickr, self.flickr()))
        if not streams:
            raise RuntimeError("No SFT data sources enabled.")

        worker = get_worker_info()
        rng = random.Random(self.args.seed + 20_000 + (worker.id if worker else 0))
        weights = [w for w, _ in streams]
        iters = [it for _, it in streams]
        while True:
...
def preview(args: argparse.Namespace, tokenizer: Any, cfg: ModelConfig) -> None:
    ds = SFTDataset(args, tokenizer, cfg)
    it = iter(ds)
    for i in range(args.preview):
        ...
        target = tokenizer.decode([x for x in labels if x != -100], skip_special_tokens=False)
        print(f"\n## Sample {i + 1}")
        print(text[:1200])
        print("-- target --")
        print(target[:600])
        print(f"image={sample['image'] is not None} image_position={sample['image_position']}")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--checkpoint", required=True, help="Base clean-pretrain checkpoint, e.g. /workspace/runs/tiny-mm-clean/ckpt_last.pt")
    p.add_argument("--tokenizer_dir", default="/workspace/tokenizer_8k_clean")
    p.add_argument("--out_dir", default="/workspace/runs/tiny-mm-clean-sft")
    p.add_argument("--resume_from", default=None, help="Resume an interrupted SFT checkpoint.")
    p.add_argument("--jsonl", nargs="*", default=[], help="One or more get2.py JSONL files.")
    p.add_argument("--image_base_dir", nargs="*", default=["."], help="Base dirs for resolving row['image'] paths.")
    ...
    p.add_argument("--max_answer_words", type=int, default=90)
    p.add_argument("--shuffle_buffer", type=int, default=5000)
    p.add_argument("--image_shuffle_buffer", type=int, default=2000)

    p.add_argument("--mix_jsonl", type=float, default=0.80)
    p.add_argument("--mix_tinystories", type=float, default=0.05)
    p.add_argument("--mix_coco", type=float, default=0.10)
    p.add_argument("--mix_flickr", type=float, default=0.05)
    return p.parse_args()


def load_base_checkpoint(path: str, device: torch.device) -> Tuple[ModelConfig, Dict[str, torch.Tensor], Dict[str, Any]]:
    ckpt = torch.load(path, map_location=device)
    cfg_dict = ckpt.get("model_config") or {}
    cfg = ModelConfig(**cfg_dict)
    return cfg, ckpt["model"], ckpt
...
def main() -> None:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    with (out_dir / "sft_args.json").open("w") as f:
    ...
    cfg, base_state, _ = load_base_checkpoint(args.checkpoint, device)
    model = TinyNativeMultimodalGPT(cfg, pad_id=pad_id).to(device)
    model.load_state_dict(base_state)
    print(f"Loaded base {args.checkpoint}")
    ...
    dataset = SFTDataset(args, tokenizer, cfg)
    loader = DataLoader(
        dataset,
    ...
    if args.resume_from:
        ...
        print(f"Resumed SFT at step {start_step}. Training until {args.max_steps}.")
    ...
    t0 = time.time()
    pbar = tqdm(range(start_step, args.max_steps), desc="sft", dynamic_ncols=True)
    for step in pbar:
        lr = get_lr(step, args)
